...
- (void)customNavigationBar
{
    ...
    [left addTarget:self action:@selector(leftBarClick:) forControlEvents:UIControlEventTouchUpInside];
...
- (void)captureOutput:(AVCaptureOutput *)captureOutput didOutputMetadataObjects:(NSArray *)metadataObjects fromConnection:(AVCaptureConnection *)connection {
    ...
    if (metadataObjects.count>0) {
        ...
        NSLog(@"metadataObjects = %@----%@", metadataObjects,obj.stringValue);
        if ([JudgePhone judgePhoneNumber:obj.stringValue]) {
            NSLog(@"手机号正确--%@",obj.stringValue);
            FriendInfoViewController *info = [[FriendInfoViewController alloc]init];
            info.phone = obj.stringValue;
            info.type = @"addFriend";
            //info.hidesBottomBarWhenPushed = YES;
            PushController(info)
        }else{
            ScanResultViewController *res = [[ScanResultViewController alloc]init];
            res.res = obj.stringValue;
            PushController(res)
        }
    }
//    if (metadataObjects.count > 0) {
