...
-(NSString *)yearAndMonth{
    NSDateComponents *components = [CURRENT_CALENDAR components:DATE_COMPONENTS fromDate:self];
    return [NSString stringWithFormat:@"%ld.%ld",(long)components.year,components.month];
}
#pragma mark--here获取时间自定义方法
+(NSString *)myhereDateFromTimeinterval:(NSTimeInterval)timeInterval{
    NSDateFormatter *dateFormatter = [NSDateFormatter dateFormatterWithFormat:@"yyyy.MM.dd HH:mm"];
    NSDate *date = [NSDate dateWithTimeIntervalSince1970:timeInterval /1000];
    return [dateFormatter stringFromDate:date];
}
#pragma mark--我的评论获取时间自定义方法
+(NSString *)myCommentDateFromTimeinterval:(NSTimeInterval)timeInterval{
    NSDateFormatter *dateFormatter = [NSDateFormatter dateFormatterWithFormat:@"yyyy.MM.dd"];
    NSDate *date = [NSDate dateWithTimeIntervalSince1970:timeInterval/1000];
