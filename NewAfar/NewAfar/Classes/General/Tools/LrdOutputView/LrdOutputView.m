...
- (instancetype)initWithDataArray:(NSArray *)dataArray
    ...
    if (self = [super initWithFrame:CGRectMake(0, 0, SCREEN_WIDTH, SCREEN_HEIGHT)]) {
       ...
       // [_tableView registerClass:[UITableViewCell class] forCellReuseIdentifier:@"cell"];
        [_tableView registerNib:[UINib nibWithNibName:@"LrdOutputCell" bundle:nil] forCellReuseIdentifier:@"cell"];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    LrdOutputCell *cell = [tableView dequeueReusableCellWithIdentifier:@"cell"];
...
//    cell.textLabel.font = [UIFont systemFontOfSize:15];
    ...
    if ([model.imageName isEqualToString:@"empty"]) {
        cell.icon.hidden = YES;
        cell.name.sd_layout.spaceToSuperView(UIEdgeInsetsMake(0, 10, 0, 0));
    }else{
        cell.icon.hidden = NO;
        [cell.icon sd_setImageWithURL:[NSURL URLWithString:model.imageName] placeholderImage:[UIImage imageNamed:@"avatar_zhixing"]];
    }
