...
- (void)viewDidLoad {
    ...
    [self customNavigationBar:@"我的日志" hasLeft:YES hasRight:NO withRightBarButtonItemImage:@""];
...
- (void)backPage{
    [super backPage];
    if ([flagStr isEqualToString:@"保存成功！"]) {
...
- (void)initUI{
    ...
    _edit.frame = CGRectMake(ViewWidth-50, 160, 30, 30);
    [_edit setBackgroundImage:[UIImage imageNamed:@"pen"] forState:UIControlStateNormal];
    [_edit addTarget:self action:@selector(editClick:) forControlEvents:UIControlEventTouchUpInside];
    [imv addSubview:_edit];
    _edit.hidden = _isMiss;
    NSLog(_edit.hidden?@"YES":@"NO");
...
- (void)editClick:(UIButton *)sender{
    ...
    [formatter setTimeZone:[NSTimeZone timeZoneWithAbbreviation:@"UTC"]];
    //NSString转NSDate
    NSDate *dateS = [formatter dateFromString:choseDate];
    ...
    if (result == NSOrderedSame) {
        log.c_newid = _c_newid;
    }else{
       log.c_newid = @"";
    }
    if (![titleLabel.text isEqualToString:@"无记录"]) {
        log.log_content = titleLabel.text;
    }
    // 直接调用 detail的 block 实现
    log.block = ^(NSString *str){
        
        flagStr = str;
        // 打印传进来的 str
        NSLog(@"str :%@",str);
        if ([str isEqualToString:@"保存成功！"]) {
            [self sendRequestLogData:choseDate];
        }
        
    };
    BaseNavigationController *navc = [[BaseNavigationController alloc]initWithRootViewController:log];
...
//    NSString *string = [NSString stringWithFormat:str,date,[userDefault valueForKey:@"name"],UUID];
    ...
    NSString *string = [NSString stringWithFormat:str,date,[userDefault valueForKey:@"name"],[userDefault valueForKey:@"name"],UUID];
...
- (void)sendRequestHadReadData{
    ...
    NSString *string = [NSString stringWithFormat:str,_c_newid,[userDefault valueForKey:@"name"],UUID];
    [NetRequest sendRequest:BaseURL parameters:string success:^(id responseObject) {
        NSData *data = (NSData *)responseObject;
        DDXMLDocument *doc = [[DDXMLDocument alloc]initWithData:data options:0 error:nil];
        NSLog(@"%@",doc);
